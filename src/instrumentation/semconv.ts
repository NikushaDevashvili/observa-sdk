...
export function mapOpenAIToOTEL(request: any, response: any) {
  const isResponses = response?.object === "response";
  return {
    ...
    [OTEL_SEMCONV.GEN_AI_USAGE_INPUT_TOKENS]: isResponses
      ? response?.usage?.input_tokens
      : response?.usage?.prompt_tokens,
    [OTEL_SEMCONV.GEN_AI_USAGE_OUTPUT_TOKENS]: isResponses
      ? response?.usage?.output_tokens
      : response?.usage?.completion_tokens,
    [OTEL_SEMCONV.GEN_AI_FINISH_REASONS]: isResponses
      ? response?.status
        ? [response.status]
        : null
      : response?.choices?.map((c: any) => c.finish_reason),
  };
}
...
export function mapAnthropicToOTEL(request: any, response: any) {
  return {
    ...
    [OTEL_SEMCONV.GEN_AI_FINISH_REASONS]: response?.stop_reason
